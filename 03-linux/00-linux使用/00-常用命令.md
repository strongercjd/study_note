...
切换为粘贴模式，防止格式混乱 `set paste` 适用于大量粘贴，退出的话，直接退出vim，再进vim就可以了
...
- **VSZ**：虚拟内存大小（822 MB）是进程在用户空间里能看到的、已经分配的虚拟地址空间总量。它不是实际占用的物理内存，而是“进程自己认为自己拥有的内存空间大小”。也就是代码melloc了多少内存，单是内存可能没有实际写入数据
- **%VSZ**：VSZ 是进程虚拟内存使用率，表示该进程的虚拟内存大小（VSZ）占系统总虚拟内存的百分比 
> bosbox版本中 %VSZ = (进程 VSZ / 系统总物理内存 RAM) × 100%
