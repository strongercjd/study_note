...
这个重载符，也是成员函数，只是这样写方便程序书写，理解，编译器可能会编译出如下函数（只是举例子，实际编译器有自己的规则）

```cpp
  // 重载 + 运算符，用于把两个 Box 对象相加
  Box operator_+_fun(const Box& b)
  {
     Box box;
     box.length = this->length + b.length;
     box.breadth = this->breadth + b.breadth;
...
```
// 把两个对象相加，得到 Box3
Box3 = Box1 + Box2;//程序的书写

Box3 = Box1.operator_+_fun(Box2);//编译器实际执行的
// 返回的Box就返回给了Box3，入参就是Box2，this指的就是Box1
Box operator+(const Box& b)
{
    Box box;
    box.length = this->length + b.length;
...
int main()
{
    ...
    double avg;
    avg = double(pa1);//将类pa1强制转换为double类型，就会调用pa1类中的 pa2.operator double();
    std::cout << "Average of pa1 is " << avg << '\n';

    double avg2;
    avg2 = pa2.operator double();// 直接调用函数
    std::cout << "Average of pa2 is " << avg2 << '\n';

    pa2.n = 20;
    pa2.visits = 5;

    double avg3 = pa2;// 这个也是强制类型转换，只是隐式转换
    std::cout << "Average of pa2 is " << avg3 << '\n';

    pa2.n = 20;
    printf("pa2.n = 20; ");
    if (pa2)// 强制转换为bool型，也是隐式转换
    {
        printf("par2 is true \r\n");
    }
    else
    {
        printf("par2 is false \r\n");
    }
...
Average of pa2 is -1
Average of pa2 is 4
pa2.n = 20; par2 is true 
pa2.n = 0; par2 is false
```
